...
<thead><tr><th>Name</th><th>Sport</th><th>Mitglied seit</th></tr></thead>
<tbody id="members-tbody"><tr><td colspan="3">Lade Daten…</td></tr></tbody>
</table>
</div></div>
</section>`;
}
...
function renderBlock(block) {
  ...
  switch (block.type) {
    ...
    case 'members_table': return renderMembersTable();
...
(function(){
  ...
  (async function(){
    try {
      ...
      tbody.innerHTML = (j.members||[]).map(function(m){
        return '<tr><td>'+m.name+'</td><td>'+m.sport+'</td><td>'+m.since+'</td></tr>';
      }).join('');
    } catch(e){}
  })();` : ''}
  ${needsHeroFx ? `
...
module.exports = { BLOCK_TYPES, renderBlocks, renderLayout, pageFeatures };
