...
const BLOCK_TYPES = [
  ...
  {
    type:  'form_embed',
    label: 'Formular einbetten',
    icon:  '📝',
    fields: [
      { key: 'form_slug', label: 'Formular-Slug (z.B. bootsliegeplaetze)', type: 'text' },
      { key: 'title',     label: 'Überschrift (optional, überschreibt Formular-Titel)', type: 'text' },
    ],
  },
  {
...
// Embedded form block. Renders a shell <div> with data-form-slug; the layout
// script (gated on needsForms) fetches the form definition and rows from
// /api/forms/:slug and builds the table+form client-side.
function renderFormEmbed(d) {
  const slug = (d.form_slug || '').trim();
  if (!slug) {
    return `<section class="cms-form-embed cms-form-embed--invalid"><p><em>Kein Formular-Slug angegeben.</em></p></section>`;
  }
  const title = d.title ? `<h3 class="cms-form-title">${escapeHtml(d.title)}</h3>` : '';
  return `<section class="cms-form-embed" data-form-slug="${attr(slug)}">
${title}
<div class="cms-form-loading">Lade Formular…</div>
</section>`;
}
...
function renderBlock(block) {
  ...
  switch (block.type) {
    ...
    case 'form_embed':    return renderFormEmbed(d);
...
function renderNav(nav, user) {
  ...
  const auth = user
    ? `<span class="auth-user">${escapeHtml(user.username)}</span>
       ${['admin','dbadmin','webadmin'].includes(user.role) ? '<a class="btn" href="/admin">Admin</a>' : ''}
...
// ── Theme defaults ─────────────────────────────────────────────────────────
// When editors haven't set a colour/font yet the layout falls back to these.
const THEME_DEFAULTS = {
  color_primary:     '#18a558',
  color_primary_ink: '#0f7a3f',
  color_accent:      '#25c2a0',
  font_family:       'Noto Sans',
  favicon_url:       '',
};

// Font-families the theme picker offers. The key is the display label, the
// value is what goes into the Google Fonts URL query-string (`family=…`).
const THEME_FONTS = {
  'Noto Sans':       'Noto+Sans:wght@400;600;800',
  'Inter':           'Inter:wght@400;600;800',
  'Open Sans':       'Open+Sans:wght@400;600;800',
  'Roboto':          'Roboto:wght@400;500;700',
  'Lato':            'Lato:wght@400;700;900',
  'Montserrat':      'Montserrat:wght@400;600;800',
  'Poppins':         'Poppins:wght@400;600;700',
  'Raleway':         'Raleway:wght@400;600;800',
  'Source Sans 3':   'Source+Sans+3:wght@400;600;800',
  'PT Sans':         'PT+Sans:wght@400;700',
  'Nunito':          'Nunito:wght@400;600;800',
};

function renderLayout({ title, body, nav, footerNav, branding, user, needsCalendar, needsMembersData, needsHeroFx, needsLightbox, needsLivestream, needsForms }) {
  const scripts = [];
...
(function(){
  ...
  if (needsForms) {
    // Client-side form loader for embedded form blocks. For each
    // <section data-form-slug="…"> it fetches the form definition + rows
    // and builds a data table with inline editing + an "add row" form.
    scripts.push(`<script>
(function(){
  var shells = document.querySelectorAll('[data-form-slug]');
  ...
  shells.forEach(function(shell){
    var slug = shell.dataset.formSlug;
    loadForm(shell, slug);
  });
  async function loadForm(shell, slug){
    try {
      var def = await api('/api/forms/'+encodeURIComponent(slug));
      var form = def.form;
      // Title (only if not already set by block)
      if (!shell.querySelector('.cms-form-title') && form.title) {
        var t = document.createElement('h3');
        t.className = 'cms-form-title';
        t.textContent = form.title;
        shell.insertBefore(t, shell.firstChild);
      }
      if (form.description) {
        var p = document.createElement('p');
        p.className = 'cms-form-desc';
        p.textContent = form.description;
        shell.appendChild(p);
      }
      // Remove loading indicator
      var ld = shell.querySelector('.cms-form-loading');
      if (ld) ld.remove();
      // Container for table + form
      var container = document.createElement('div');
      container.className = 'cms-form-container';
      shell.appendChild(container);
      await renderFormData(container, form, slug);
    } catch(err) {
      shell.innerHTML = '<p class="cms-form-error">Formular konnte nicht geladen werden: '+esc(err.message)+'</p>';
    }
  }
  async function renderFormData(container, form, slug){
    container.innerHTML = '';
    var res = await api('/api/forms/'+encodeURIComponent(slug)+'/rows?limit=200');
    var rows = res.rows, pk = res.pk, fields = form.fields;
    // Data table
    if (rows.length) {
      var tbl = document.createElement('table');
      tbl.className = 'cms-form-table';
      tbl.style.cssText = 'width:100%;border-collapse:collapse;font-size:14px;margin-bottom:16px;';
      var thRow = '<tr>';
      fields.forEach(function(f){ thRow += '<th style="padding:6px 8px;border-bottom:2px solid var(--line);text-align:left;">'+esc(f.label||f.key)+'</th>'; });
      thRow += '</tr>';
      ...
      rows.forEach(function(row){
        ...
        fields.forEach(function(f){
          ...
          td.style.cssText = 'padding:6px 8px;border-bottom:1px solid var(--line);';
    ...
    formEl.className = 'cms-form-add';
    formEl.innerHTML = '<h4 style="margin:0 0 8px;">Neuer Eintrag</h4>';
    var inputs = {};
    fields.forEach(function(f){
      var lbl = document.createElement('label');
      lbl.style.cssText = 'display:block;margin-bottom:8px;';
      lbl.textContent = (f.label||f.key) + (f.required ? ' *' : '');
      var inp;
      if (f.type === 'textarea') { inp = document.createElement('textarea'); inp.style.cssText = 'width:100%;min-height:60px;'; }
      else if (f.type === 'number') { inp = document.createElement('input'); inp.type = 'number'; inp.style.width = '100%'; }
      else if (f.type === 'date')   { inp = document.createElement('input'); inp.type = 'date'; inp.style.width = '100%'; }
      else if (f.type === 'datetime') { inp = document.createElement('input'); inp.type = 'datetime-local'; inp.style.width = '100%'; }
      else if (f.type === 'checkbox') { inp = document.createElement('input'); inp.type = 'checkbox'; }
      else { inp = document.createElement('input'); inp.type = 'text'; inp.style.width = '100%'; }
    ...
    submitBtn.addEventListener('click', async function(){
      ...
      try {
        await api('/api/forms/'+encodeURIComponent(slug)+'/rows', { method:'POST', body:JSON.stringify({data:data}) });
        await renderFormData(container, form, slug);
      } catch(err) {
  ...
  // ── Theme: resolve values from branding (editor-set) or defaults ──────────
  const t = {
    primary:     branding.color_primary     || THEME_DEFAULTS.color_primary,
    primaryInk:  branding.color_primary_ink || THEME_DEFAULTS.color_primary_ink,
    accent:      branding.color_accent      || THEME_DEFAULTS.color_accent,
    fontFamily:  branding.font_family       || THEME_DEFAULTS.font_family,
    faviconUrl:  branding.favicon_url       || THEME_DEFAULTS.favicon_url,
  };
  // Build Google Fonts URL for the selected family.
  const fontSpec = THEME_FONTS[t.fontFamily] || THEME_FONTS['Noto Sans'];
  const gFontsUrl = `https://fonts.googleapis.com/css2?family=${fontSpec}&display=swap`;
  // Favicon tag (only emitted when a URL is set).
  const faviconTag = t.faviconUrl
    ? `<link rel="icon" href="${attr(t.faviconUrl)}" />`
    : '';
  // CSS custom-property overrides injected inline so they take precedence
  // over the defaults in styles.css's :root block.
  const themeVars = `<style>:root{--green:${attr(t.primary)};--green-ink:${attr(t.primaryInk)};--aqua:${attr(t.accent)};--font:"${attr(t.fontFamily)}",system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;}</style>`;
...
<link href="${attr(gFontsUrl)}" rel="stylesheet">
...
function pageFeatures(blocks) {
  ...
  return {
    ...
    needsForms:       types.has('form_embed'),
...
module.exports = { BLOCK_TYPES, THEME_FONTS, THEME_DEFAULTS, renderBlocks, renderLayout, pageFeatures, isFullHtmlPage };
