...
function renderLayout({ title, body, nav, footerNav, branding, user, needsCalendar, needsMembersData, needsHeroFx, needsLightbox }) {
  const scripts = [];
...
(function(){
  ...
  ${needsLightbox ? `
  (function(){
    // Click-to-zoom for any <img class="cms-zoomable"> or
    // <a class="cms-zoomable-link"> rendered by the CMS. We attach a single
    // delegated listener so dynamically inserted images (e.g. CKEditor
    // content) also benefit. Original image URL comes from data-fullsrc on
    // the <img>, falling back to href on a wrapping anchor.
    function getFullSrc(target){
      var img = target.closest && target.closest('.cms-zoomable');
      if (img && img.dataset && img.dataset.fullsrc) return img.dataset.fullsrc;
      var a = target.closest && target.closest('a.cms-zoomable-link');
      if (a && a.getAttribute('href')) return a.getAttribute('href');
      // Generic fallback: any <img> inside CMS content. Skip tiny icons.
      var rawImg = target.closest && target.closest('img');
      if (rawImg && rawImg.naturalWidth > 200 && rawImg.src
          && rawImg.closest('.cms-richtext, .cms-image, .cms-gallery, .boats-grid')) {
        return rawImg.currentSrc || rawImg.src;
      }
      return null;
    }
    var overlay = null;
    ...
    function open(src){
      ...
      overlay.innerHTML = '<button class="lightbox-close" aria-label="Schließen">×</button>'
